// const activities = [
//   {
//     id: 1,
//     duration: 85,
//     price: 128,
//   },
//   {
//     id: 2,
//     duration: 235,
//     price: 218,
//   },
//   {
//     id: 3,
//     duration: 119,
//     price: 71,
//   },
//   {
//     id: 4,
//     duration: 165,
//     price: 352,
//   },
//   {
//     id: 5,
//     duration: 173,
//     price: 125,
//   },
//   {
//     id: 6,
//     duration: 58,
//     price: 174,
//   },
//   {
//     id: 7,
//     duration: 163,
//     price: 392,
//   },
//   {
//     id: 8,
//     duration: 196,
//     price: 488,
//   },
//   {
//     id: 9,
//     duration: 83,
//     price: 409,
//   },
//   {
//     id: 10,
//     duration: 120,
//     price: 399,
//   },
//   {
//     id: 11,
//     duration: 184,
//     price: 255,
//   },
//   {
//     id: 12,
//     duration: 99,
//     price: 408,
//   },
//   {
//     id: 13,
//     duration: 218,
//     price: 51,
//   },
//   {
//     id: 14,
//     duration: 188,
//     price: 277,
//   },
//   {
//     id: 15,
//     duration: 52,
//     price: 260,
//   },
//   {
//     id: 16,
//     duration: 22,
//     price: 408,
//   },
//   {
//     id: 17,
//     duration: 142,
//     price: 80,
//   },
//   {
//     id: 18,
//     duration: 124,
//     price: 485,
//   },
//   {
//     id: 19,
//     duration: 145,
//     price: 45,
//   },
//   {
//     id: 20,
//     duration: 103,
//     price: 435,
//   },
//   {
//     id: 21,
//     duration: 181,
//     price: 222,
//   },
//   {
//     id: 22,
//     duration: 66,
//     price: 284,
//   },
//   {
//     id: 23,
//     duration: 68,
//     price: 466,
//   },
//   {
//     id: 24,
//     duration: 73,
//     price: 396,
//   },
//   {
//     id: 25,
//     duration: 174,
//     price: 369,
//   },
//   {
//     id: 26,
//     duration: 133,
//     price: 113,
//   },
//   {
//     id: 27,
//     duration: 213,
//     price: 76,
//   },
//   {
//     id: 28,
//     duration: 136,
//     price: 254,
//   },
//   {
//     id: 29,
//     duration: 228,
//     price: 101,
//   },
//   {
//     id: 30,
//     duration: 151,
//     price: 60,
//   },
//   {
//     id: 31,
//     duration: 119,
//     price: 203,
//   },
//   {
//     id: 32,
//     duration: 189,
//     price: 474,
//   },
//   {
//     id: 33,
//     duration: 89,
//     price: 127,
//   },
//   {
//     id: 34,
//     duration: 194,
//     price: 380,
//   },
//   {
//     id: 35,
//     duration: 152,
//     price: 494,
//   },
//   {
//     id: 36,
//     duration: 38,
//     price: 240,
//   },
//   {
//     id: 37,
//     duration: 160,
//     price: 375,
//   },
//   {
//     id: 38,
//     duration: 56,
//     price: 499,
//   },
//   {
//     id: 39,
//     duration: 38,
//     price: 57,
//   },
//   {
//     id: 40,
//     duration: 148,
//     price: 256,
//   },
//   {
//     id: 41,
//     duration: 100,
//     price: 446,
//   },
//   {
//     id: 42,
//     duration: 125,
//     price: 257,
//   },
//   {
//     id: 43,
//     duration: 190,
//     price: 140,
//   },
//   {
//     id: 44,
//     duration: 70,
//     price: 414,
//   },
//   {
//     id: 45,
//     duration: 191,
//     price: 182,
//   },
//   {
//     id: 46,
//     duration: 19,
//     price: 130,
//   },
//   {
//     id: 47,
//     duration: 172,
//     price: 338,
//   },
//   {
//     id: 48,
//     duration: 162,
//     price: 272,
//   },
//   {
//     id: 49,
//     duration: 90,
//     price: 110,
//   },
//   {
//     id: 50,
//     duration: 191,
//     price: 337,
//   },
//   {
//     id: 51,
//     duration: 69,
//     price: 164,
//   },
//   {
//     id: 52,
//     duration: 50,
//     price: 42,
//   },
//   {
//     id: 53,
//     duration: 108,
//     price: 342,
//   },
//   {
//     id: 54,
//     duration: 117,
//     price: 359,
//   },
//   {
//     id: 55,
//     duration: 134,
//     price: 85,
//   },
//   {
//     id: 56,
//     duration: 142,
//     price: 442,
//   },
//   {
//     id: 57,
//     duration: 205,
//     price: 326,
//   },
//   {
//     id: 58,
//     duration: 58,
//     price: 130,
//   },
//   {
//     id: 59,
//     duration: 155,
//     price: 55,
//   },
//   {
//     id: 60,
//     duration: 58,
//     price: 311,
//   },
//   {
//     id: 61,
//     duration: 196,
//     price: 322,
//   },
//   {
//     id: 62,
//     duration: 161,
//     price: 181,
//   },
//   {
//     id: 63,
//     duration: 117,
//     price: 385,
//   },
//   {
//     id: 64,
//     duration: 42,
//     price: 435,
//   },
//   {
//     id: 65,
//     duration: 139,
//     price: 314,
//   },
//   {
//     id: 66,
//     duration: 193,
//     price: 337,
//   },
//   {
//     id: 67,
//     duration: 59,
//     price: 456,
//   },
//   {
//     id: 68,
//     duration: 58,
//     price: 120,
//   },
//   {
//     id: 69,
//     duration: 97,
//     price: 459,
//   },
//   {
//     id: 70,
//     duration: 97,
//     price: 179,
//   },
//   {
//     id: 71,
//     duration: 147,
//     price: 331,
//   },
//   {
//     id: 72,
//     duration: 58,
//     price: 170,
//   },
//   {
//     id: 73,
//     duration: 156,
//     price: 133,
//   },
//   {
//     id: 74,
//     duration: 179,
//     price: 142,
//   },
//   {
//     id: 75,
//     duration: 176,
//     price: 113,
//   },
//   {
//     id: 76,
//     duration: 16,
//     price: 252,
//   },
//   {
//     id: 77,
//     duration: 181,
//     price: 315,
//   },
//   {
//     id: 78,
//     duration: 181,
//     price: 78,
//   },
//   {
//     id: 79,
//     duration: 19,
//     price: 456,
//   },
//   {
//     id: 80,
//     duration: 43,
//     price: 106,
//   },
//   {
//     id: 81,
//     duration: 114,
//     price: 325,
//   },
//   {
//     id: 82,
//     duration: 180,
//     price: 250,
//   },
//   {
//     id: 83,
//     duration: 70,
//     price: 224,
//   },
//   {
//     id: 84,
//     duration: 130,
//     price: 71,
//   },
//   {
//     id: 85,
//     duration: 35,
//     price: 268,
//   },
//   {
//     id: 86,
//     duration: 80,
//     price: 205,
//   },
//   {
//     id: 87,
//     duration: 135,
//     price: 206,
//   },
//   {
//     id: 88,
//     duration: 135,
//     price: 470,
//   },
//   {
//     id: 89,
//     duration: 80,
//     price: 344,
//   },
//   {
//     id: 90,
//     duration: 40,
//     price: 353,
//   },
//   {
//     id: 91,
//     duration: 160,
//     price: 269,
//   },
//   {
//     id: 92,
//     duration: 140,
//     price: 441,
//   },
//   {
//     id: 93,
//     duration: 150,
//     price: 62,
//   },
//   {
//     id: 94,
//     duration: 180,
//     price: 445,
//   },
//   {
//     id: 95,
//     duration: 60,
//     price: 495,
//   },
//   {
//     id: 96,
//     duration: 120,
//     price: 351,
//   },
//   {
//     id: 97,
//     duration: 50,
//     price: 400,
//   },
//   {
//     id: 98,
//     duration: 200,
//     price: 127,
//   },
//   {
//     id: 99,
//     duration: 120,
//     price: 490,
//   },
//   {
//     id: 100,
//     duration: 90,
//     price: 343,
//   },
// ];

const activities = [
  {
    id: 1,
    duration: 100,
    price: 200,
  },
  {
    id: 2,
    duration: 250,
    price: 225,
  },
  {
    id: 3,
    duration: 300,
    price: 300,
  },
  {
    id: 4,
    duration: 80,
    price: 50,
  },
  {
    id: 5,
    duration: 70,
    price: 10,
  },
  {
    id: 6,
    duration: 240,
    price: 300,
  },
];

const bestPlan = (days, budget, timeForActivities) => {
  // Offer to client
  let offer = [];

  // Define how many hours are available for the entire holiday that can be used for activites
  const minutesAvailable = days * (timeForActivities * 60);

  console.log("Time for activities: " + minutesAvailable + " minutes. With a budget of " + budget);

  // Excluding activities with price over the budget
  const arrayFiltered = activities.filter((activity) => {
    return activity.price < budget;
  });

  console.log("Activities within the budget:", arrayFiltered);

  // Sorting array from lower to higher duration
  arrayFiltered.sort((a, b) => {
    if (a.duration === b.duration) {
      return a.price - b.price;
    }
    return a.duration > b.duration ? 1 : -1;
  });

  console.log("Sorting list by duration. If 2 or more activities have same duration, order from lower to higher price:", arrayFiltered);

  let sum = 0;
  let sumMinutes = 0;

  for (let activity of arrayFiltered) {
    sum += activity.price;
    sumMinutes += activity.duration;
    console.log(sum);
    console.log(sumMinutes);
    if (sum > budget || sumMinutes >= minutesAvailable) {
      console.log("stop");
      console.log(offer);
      return;
    }
    offer.push(activity);
  }
};

const days = 3;
const budget = 560;
const timeForActivities = 4; // In hours per days

bestPlan(days, budget, timeForActivities);
